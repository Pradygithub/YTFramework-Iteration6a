...
import org.testing.pages.Commentpost;
...
public class TC7 extends Base
{
	...
	@Test
	public void commentPost() throws InterruptedException, IOException
	{
		//Login & Capture logs
		Signin s=new Signin(driver, pr);
		s.login("panwarp727@gmail.com", "pradygoogle");
		LogsCapture.takeLogs("TC7", "Login success");
		
		//URL matching using hard assertion
		 String expected_url="https://www.youtube.com/";             
		 String actual_url=driver.getCurrentUrl(); 
		 Assert.assertEquals(expected_url, actual_url);
		 LogsCapture.takeLogs("TC7", "Hard assertion for URL matching is true");
		
		//screenshot capture 
		String mypath="E:\\screenshots\\"+"TC7_"+DateCalculation.calculateDate()+".png";
		...
		// comment posting
		Commentpost cp=new Commentpost(driver, pr);
		cp.postComment();
		
		//Logout and capture logs
		Signout s1=new Signout(driver, pr);
		s1.logout();
		LogsCapture.takeLogs("TC7", "Logout success");
